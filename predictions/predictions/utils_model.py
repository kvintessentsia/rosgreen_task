...
for filename in os.listdir(path='./aclImdb/train/neg'):
    ...
    train_negative_labels.append(int(label))

train_positive_labels = []

for filename in os.listdir(path='./aclImdb/train/pos'):
    label = filename.split('_')[1].split('.')[0]
    train_positive_labels.append(int(label))

test_negative_labels = []

for filename in os.listdir(path='./aclImdb/test/neg'):
    label = filename.split('_')[1].split('.')[0]
    test_negative_labels.append(int(label))

test_positive_labels = []

for filename in os.listdir(path='./aclImdb/test/pos'):
    label = filename.split('_')[1].split('.')[0]
    test_positive_labels.append(int(label))
...
train_neg_labels = np.array(train_negative_labels)
train_pos_labels = np.array(train_positive_labels)
test_neg_labels = np.array(test_negative_labels)
test_pos_labels = np.array(test_positive_labels)
...
UNIQUE_LABELS_N = len(set(train_labels)) + 3
...
joblib.dump(test_dataset, "test_dataset_new.pkl")
